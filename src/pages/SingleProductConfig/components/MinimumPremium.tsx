...
const MinimumPremium: React.FC<MinimumPremiumProps> = ({
  isLoading,
  error,
  projectTypesMasters,
  activeProjectTypes,
    ...
    <Card className="h-full">
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle>Minimum Premium</CardTitle>
            <CardDescription>Configure minimum premium rates for different sub-project types</CardDescription>
          </div>
          <Button onClick={onSave} size="sm" disabled={isLoading || isSaving}>
            ...
            {isLoading ? 'Loading...' : isSaving ? 'Saving...' : 'Save Minimum Premium'}
          </Button>
        </div>
      </CardHeader>
      <CardContent>
        {isLoading && (
          <div className="space-y-6">
            {/* Flattened Minimum Premium Table Skeleton */}
